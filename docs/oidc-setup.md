...
| コンソール操作が好き・CLI を使いたくない | **方法B: AWS コンソールで手動作成** ⭐ AWS推奨 |
| チームで IaC の再現性を重視したい | **方法C: 一時キーで bootstrap して即削除** |

> **AWS 公式推奨**: [AWS Security Blog](https://aws.amazon.com/blogs/security/use-iam-roles-to-connect-github-actions-to-actions-in-aws/) では「コンソール手動」または「AWS CLI（IAM コマンド直接実行）」を推奨しています。CDK を使う方法A・Cは AWS 公式手順には含まれませんが、結果は同じです。
